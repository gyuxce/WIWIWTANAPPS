...
interface AssesmentActionSheetProps {
  ...
  query: { q: string; lowest: string; highest: string };
  setQuery: any;
}

const AssesmentActionSheet = ({
  actionSheetRef,
  snapPoints,
  setSelectedSort,
  dataSort,
  ...
  query,
  setQuery,
}: AssesmentActionSheetProps) => {
  const { t } = useTranslation();
  const { bottom } = useSafeAreaInsets();
  const [sort, setSort] = useState({} as { id: string; title: string });
  const [filter, setFilter] = useState([] as { id: string; title: string }[]);
  ...
  const [queryAction] = useState(query);
        ...
        >
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              >
                <TextInput
                  value={queryAction?.lowest}
                  onChangeText={text => {
                    setQueryAction({ ...queryAction, lowest: text });
                  }}
                  style={{
                    width: "100%",
                    fontSize: 12,
            ...
            <View style={{ flex: 1 }}>
              ...
              >
                <TextInput
                  value={queryAction?.highest}
                  onChangeText={text => {
                    setQueryAction({ ...queryAction, highest: text });
                  }}
                  style={{
                    width: "100%",
                    fontSize: 12,
          ...
          <Button
            onPress={() => {
              ...
              if (date?.type === "custom") {
                setSelectedDate({
                  ...selectedDate,
                  id: `filter,event.started_at,between,${dayjs(
                    date?.start_date,
                  ).format("YYYY-MM-DD")}|${dayjs(date?.end_date).format(
                    "YYYY-MM-DD",
                  )}`,
                });
              } else {
...
export default AssesmentActionSheet;
