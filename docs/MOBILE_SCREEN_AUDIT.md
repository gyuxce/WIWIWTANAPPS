...
- Perbaikan seed lokal: `UpdateCourseJapaneseTitlesSeeder` ditambahkan dan dipanggil dari `DevDatabaseSeeder` setelah import Excel. Database audit lokal aktif sudah terisi `日本語理論`, `日本語実践`, dan `日本語ソフトスキル` untuk tiga kategori course utama.
- Verifikasi emulator: setelah login ulang siswa `user1@62teknologi.com`, endpoint `/mobile/training/module/progress` mengembalikan `title_japan` untuk tiga course utama. Layar Progress menampilkan kartu course `日本語理論` dan `日本語実践`; Detail Training untuk course pertama menampilkan judul `日本語理論` dengan progress `5 / 20`.
- Catatan verifikasi Home: Home terbuka tanpa crash, tetapi viewport run ini berada di section forum/seminar sehingga kartu course tidak terlihat pada screenshot. Karena Home memakai komponen `SectionLesson` yang sama dengan Progress untuk render course card, hasil Progress memvalidasi jalur render title course yang sama.
- Course items/module/virtual/assessment: tahap awal sudah diimplementasikan dengan kolom `course_items.title_japan`, update `CourseItemResource`, request validation, CMS Modul Pelatihan, export field, seed lokal, dan mobile render fallback `title_japan || title` pada tab Modul. Route param internal `title` tetap memakai judul asli karena masih dipakai untuk matching assessment.
