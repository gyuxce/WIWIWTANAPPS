...
export const Tools = (props) => {
  ...
  const [checkPerihal, SetCheckPerihal] = useState([]);
  ...
  const submitFilter = () => {
    ...
    if (checkPerihal.length > 0) {
      options.push(`filter,type,in,${checkPerihal.join('|')}`);
    }

    if (checkStatus.length > 0) {
      options.push(`filter,status,in,${checkStatus.join('|')}`);
    }
    let params = {
      ...props.localState.params,
      siswa_role: checkRole,
    };
    props.getData({ ...params, options: options });
    ...
    <>
          ...
          {PageConfig.enableFilterTools && (
            ...
            >
              <div className="flex flex-row justify-between">
                ...
                {checkPerihal.length + checkRole.length + checkStatus.length > 0 && (
                  <div className="h-auto w-5 bg-main-200 rounded flex justify-center items-center">
                    <p className="text-sub-b text-white -mb-[2px] text-[12px]">
                      {checkPerihal.length + checkRole.length + checkStatus.length}
                    </p>
                  </div>
                )}
              </div>
      ...
      >
        <div className="h-[85%] 2xl:h-[88%] overflow-auto">
          <AccordionCustome>
            <AccordionItemCustome title="Role">
              <div className="mt-3 py-3 px-4">
                <Checkbox.Group vertical value={checkRole} onChange={SetCheckRole}>
                  {props.role?.map((val, idx) => {
                    return (
                      <Checkbox
                        key={idx}
                        className={`text-center text-black text-sm font-normal font-goth leading-[21px] ${
                          idx != props.role.length - 1 ? ' mb-5' : ''
                        }`}
                        value={val.id}
                      >
                        {val.name}
          ...
          <AccordionCustome>
            <AccordionItemCustome title="Perihal">
              <div className="mt-3 py-3 px-4">
                <Checkbox.Group vertical value={checkPerihal} onChange={SetCheckPerihal}>
                  {props.constants?.map((val, idx) => {
                    return (
                      <Checkbox
                        key={idx}
                        className={`text-center text-black text-sm font-normal font-goth leading-[21px] ${
                          idx != props.constants.length - 1 ? ' mb-5' : ''
                        }`}
                        value={val.value}
                      >
                        {val.name}
          ...
          <AccordionCustome>
            <AccordionItemCustome title="Status Laporan">
              <div className="mt-3 py-3 px-4">
                <Checkbox.Group vertical value={checkStatus} onChange={SetCheckStatus}>
                  {props.status?.map((val, idx) => {
                    return (
                      <Checkbox
                        key={idx}
                        className={`text-center text-black text-sm font-normal font-goth leading-[21px] ${
                          idx != props.status.length - 1 ? ' mb-5' : ''
                        }`}
                        value={val.value}
                      >
                        {val.name}
        ...
        <div className="">
          <div className="flex items-center justify-center border-t border-stone-200 space-x-3 pt-6 pb-8">
            ...
            <Button
              variant="default"
              className="!py-0"
              style={{ height: '40px', width: '153px' }}
              icon={<PlusCircledIcon style={{ height: '24px', width: '24px' }} />}
