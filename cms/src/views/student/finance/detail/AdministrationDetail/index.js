...
const AdministrationDetail = () => {
  ...
  const [localState, setLocalState] = useState({
    ...
    updated: null,
    params: {
      relations: ['transactionAdministration.payments.paymentProof.file'].join(),
    },
  });
  ...
  const fetchData = async () => {
    try {
      const params = {
        relations:
          'user, transactionAdministration.payments.paymentProof, transactionAdministration.payments.paymentDetail',
      };
    ...
    async (params) => {
      try {
        ...
        setLocalState({
          ...localState,
          loading: false,
          data: ress.data?.data?.transactionAdministration?.payments || [],
    ...
    <div className="">
      <div className="rounded-2xl border-none bg-white">
        <Card className="border-none rounded-xl">
          <div className=" text-black text-xl font-normal font-opificio mb-6">Administrasi</div>
          <div className="flex justify-between items-center text-black text-sm font-normal font-goth leading-[21px] pb-3 border-b border-stone-200 mb-5">
            ...
            <div className="">
              {data.transactionAdministration != null
                ? (() => {
                    const color =
                      data.transaction_status == 1 ? 'green' : data.transaction_status == 2 ? 'orange' : 'red';

                    return (
                      <span
                        className={`px-3 py-1 bg-${color}-50 rounded-full border border-${color}-500 text-${color}-500 text-xs font-normal font-goth`}
                      >
                        {data.transaction_status_administration}
                      </span>
                    );
                  })()
                : '-'}
          ...
          <div className="flex justify-between items-center text-black text-sm font-normal font-goth leading-[21px] pb-3 border-b border-stone-200 mb-5">
            ...
            <span>
              {data.transactionAdministration?.payments.length > 1
                ? data?.transactionAdministration?.payments[0]?.paymentProof?.created_at
                  ? dayjs
                      .utc(data?.transactionAdministration?.payments[0]?.paymentProof?.created_at)
                      .format('D MMM YYYY')
                  : '-'
                : data?.transactionAdministration?.payments[0]?.paymentDetail?.paid_at
                ? dayjs.utc(data?.transactionAdministration?.payments[0]?.paymentDetail?.paid_at).format('D MMM YYYY')
                : '-'}
            </span>
          </div>
          <div className="flex justify-between items-center text-black text-sm font-normal font-goth leading-[21px] pb-3 border-b border-stone-200 mb-5">
          ...
          <div className="flex justify-between items-center text-black text-sm font-normal font-goth leading-[21px] pb-3 border-b border-stone-200 mb-5">
            ...
            {data.transactionAdministration != null
              ? (() => {
                  return (
                    <span>
                      {data.transactionAdministration?.payments.length == 6 ? 'Cicilan Pribadi' : 'Pembayaran Lunas'}
                    </span>
                  );
                })()
              : '-'}
          ...
          <div className="flex justify-between items-center text-black text-sm font-normal font-goth leading-[21px] pb-3 border-b border-stone-200 mb-5">
            ...
            {data.transactionAdministration != null
              ? (() => {
                  return <span>{numberToRupiah(data.transactionAdministration?.total)}</span>;
                })()
              : '-'}
          </div>
          ...
          {data.transactionAdministration?.payments.length == 1 ? (
            <div className="flex justify-between items-center text-black text-sm font-normal font-goth leading-[21px] pb-3 border-b border-stone-200 mb-5">
              ...
              <span>{data.transactionAdministration?.payments[0]?.paymentDetail?.number}</span>
            </div>
          ) : null}
