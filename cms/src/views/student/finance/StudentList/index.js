...
const Page = () => {
  ...
  const [transactionAdminstrationConstant, setTransactionAdminstrationConstantt] = useState([]);
  const [transactionTrainingConstant, setTransactionTrainingConstantt] = useState([]);
  ...
  const getTransactionAdminstrationConstant = async () => {
    try {
      const ress = await apiGetConstants({ data: 'transaction_administration_status' });
      setTransactionAdminstrationConstantt(ress.data || []);
    } catch (error) {
      toast.push(
        <Notification title={'Error'} type="danger">
          {error?.response?.data?.message || error?.message || 'Something went wrong'}
  ...
  const getTransactionTrainingConstant = async () => {
    try {
      const ress = await apiGetConstants({ data: 'transaction_training_status' });
      setTransactionTrainingConstantt(ress.data || []);
    } catch (error) {
      toast.push(
        <Notification title={'Error'} type="danger">
          {error?.response?.data?.message || error?.message || 'Something went wrong'}
  ...
  useEffect(() => {
    if (firstLoad.current) {
      ...
      getTransactionAdminstrationConstant();
      getTransactionTrainingConstant();
      ...
      <div className="rounded-2xl border-none p-6 bg-white">
        ...
        <div className="mb-[1.25rem]">
          <Tools
            ...
            transactionAdminstrationConstant={transactionAdminstrationConstant}
            transactionTrainingConstant={transactionTrainingConstant}
