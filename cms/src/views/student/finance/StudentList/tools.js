...
export const Tools = (props) => {
  ...
  const handleCheckboxChangeAdmin = (value) => {
    if (checkTransactionAdminstration.includes(value)) {
      SetCheckTransactionAdminstration(checkTransactionAdminstration.filter((item) => item !== value));
    } else {
      SetCheckTransactionAdminstration([...checkTransactionAdminstration, value]);
    }
  };

  const handleCheckboxChangeTraining = (value) => {
    if (checkTransactionTraining.includes(value)) {
      SetCheckTransactionTraining(checkTransactionTraining.filter((item) => item !== value));
    } else {
      SetCheckTransactionTraining([...checkTransactionTraining, value]);
    }
  };
    ...
    <>
      ...
      >
        <div className="h-[85%] 2xl:h-[88%] overflow-auto">
          ...
          <AccordionCustome>
            <AccordionItemCustome title="Status Pembayaran Administrasi">
              <div className="mt-3 py-3 px-4">
                {props.transactionAdminstrationConstant?.map((val, idx) => {
                  const isCheckedAdmin = checkTransactionAdminstration.includes(val.value);

                  return (
                    <div key={idx}>
                      <Checkbox
                        checked={isCheckedAdmin}
                        className={`${idx != props.transactionAdminstrationConstant?.length - 1 ? ' mb-5' : ''}`}
                        onChange={() => handleCheckboxChangeAdmin(val.value)}
                      >
                        <span className="text-center text-black text-sm font-normal font-goth leading-[21px]">
                          {val.name}
                        </span>
          ...
          <div className="h-4"></div>
          <AccordionCustome>
            <AccordionItemCustome title="Status Pembayaran Pelatihan">
              <div className="mt-3 py-3 px-4">
                {props.transactionTrainingConstant?.map((val, idx) => {
                  const isCheckedTraining = checkTransactionTraining.includes(val.value);

                  return (
                    <div key={idx}>
                      <Checkbox
                        className={`${idx != props.transactionTrainingConstant?.length - 1 ? ' mb-5' : ''}`}
                        checked={isCheckedTraining}
                        onChange={() => handleCheckboxChangeTraining(val.value)}
                      >
                        <span className="text-center text-black text-sm font-normal font-goth leading-[21px]">
                          {val.name}
                        </span>
