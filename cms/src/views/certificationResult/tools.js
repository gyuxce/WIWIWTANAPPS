...
export const Tools = (props) => {
  ...
  const submitFilter = async () => {
    ...
    if (startDate) {
      params.options.push(
        'filter,updated_at,greater_than_equal,' + dayjs(startDate).format('YYYY-MM-DD') + ' 00:00:00',
      );
    }
    if (endDate) {
      params.options.push('filter,updated_at,less_then_equal,' + dayjs(endDate).format('YYYY-MM-DD') + ' 23:59:59');
    }
  ...
  const handleExport = async () => {
    setIsExporting(true);
    await exportData('xlsx', apiExport, 'hasil_sertifikasi', props.localState.params);
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-[1rem]">
          ...
          >
            <div className="flex flex-row justify-between">
              ...
              {props.localState.params.options.length > 0 || checkboxSorting ? (
                <div className="h-auto w-5 bg-main-200 rounded flex justify-center items-center">
                  <p className="text-sub-b text-white -mb-[2px] text-[12px]">
                    {props.localState.params.options.length + (checkboxSorting ? 1 : 0)}
                  </p>
                </div>
              ) : null}
            </div>
        ...
        <div className="flex justify-end items-center space-x-[0.5rem]">
          ...
          {PageConfig.enableCreate && (
            <Button
              isAction
              icon={<PlusCircledIcon height={16} width={16} />}
              type="button"
              onClick={() => navigate(PageConfig.url + '/create')}
              className="bg-white !px-3 !py-2 flex items-center !rounded"
            >
              <div className="mt-1 ml-1 !font-goth text-black !font-normal text-xs">Buat Baru</div>
            </Button>
          )}
        </div>
      </div>
