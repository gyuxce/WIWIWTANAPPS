...
export const Tools = (props) => {
  ...
  const submitFilter = async () => {
    ...
    if (startDate) {
      optionsFilter.push('filter,cert_date,greater_than_equal,' + dayjs(startDate).format('YYYY-MM-DD') + ' 00:00:00');
    }
    if (endDate) {
      optionsFilter.push(
        'filter,cert_date,less_then_equal,' + dayjs(endDate).add(1, 'day').format('YYYY-MM-DD') + ' 23:59:59',
      );
    }
    ...
    <>
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-[1rem]">
          ...
          <Button
            className={`w-[200px] h-[40px] !px-[12px] text-second-100 border-gray-300`}
            variant="plain"
            onClick={() => {
          ...
          >
            <div className="flex flex-row justify-between">
              ...
              {props.localState.params.options.length > 1 || checkboxSorting ? (
                <div className="h-auto w-5 bg-main-200 rounded flex justify-center items-center">
                  <p className="text-sub-b text-white -mb-[2px] text-[12px]">
                    {props.localState.params.options.length + (checkboxSorting ? 1 : 0) - 1}
                  </p>
                </div>
              ) : null}
            </div>
      ...
      >
        <div className="h-[85%] 2xl:h-[88%] overflow-auto">
          <Filter
            ...
            typeCertification={props.typeCertification}
          />
        </div>
        <div className="">
