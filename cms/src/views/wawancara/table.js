...
export const PageTable = (props) => {
      ...
      if (props.checkboxList.includes(el.key)) {
        if (['created_at', 'updated_at'].includes(el.key)) {
              ...
              return (
                <div className="flex text-xs">
                  {row[el.key] ? (
                    <StatusBadge
                      text={row[el.target]}
                      color={row[el.key] === 1 ? 'green' : row[el.key] === 2 ? 'red' : 'orange'}
                    />
                  ) : (
                    <>-</>
                  )}
          ...
          cols.push({
            ...
            Cell: (props) => {
              const row = props.row.original;
              const value = el?.target ? getValueByKey(row, el.target) : getValueByKey(row, el.key);
            ...
            <div className="flex flex-row justify-center items-center gap-2">
              ...
              {PageConfig.enableDelete && (
                <Button
                  ...
                  type="button"
                  onClick={() => onDelete(props?.row?.original?.user.id)}
                ></Button>
              )}
            </div>
          );
  ...
  const onEdit = (id) => {
    navigate(`${PageConfig.url}/details/${id}`);
  };
