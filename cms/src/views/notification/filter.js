...
import { OPTION_STATUS_NOTIFICATION } from 'components/ui/utils/constant';
import SideBar from 'components/template/SideFilter';
import dayjs from 'dayjs';

export const Filter = (props) => {
  const [checkboxListRepeat, setCheckboxListRepeat] = useState([]);
  ...
  const submitFilter = async () => {
    ...
    params.options = [];
    if (startDate) {
      params.options.push('filter,send_at,greater_than_equal,' + dayjs(startDate).format('YYYY-MM-DD') + ' 00:00:00');
    }
    if (endDate) {
      params.options.push('filter,send_at,less_then_equal,' + dayjs(endDate).format('YYYY-MM-DD') + ' 23:59:59');
    }
    if (checkboxListStatus.length > 0) {
      params.options.push('filter,status,in,' + checkboxListStatus.join('|'));
    }
    if (checkboxListRepeat.length > 0) {
      params.options.push('filter,repeat_each,in,' + checkboxListRepeat.join('|'));
    }
    props.getData(params);
    props.setCountFilter(checkboxListStatus.length + checkboxListRepeat.length + (startDate || endDate ? 1 : 0));
  ...
  return (
    ...
    >
      <div className="h-[85%] 2xl:h-[88%] overflow-auto">
        <AccordionCustome>
          <AccordionItemCustome title="Pengulangan">
            <Checkbox.Group
              className="mt-3 py-3 px-4"
              vertical
              value={checkboxListRepeat}
              onChange={setCheckboxListRepeat}
            >
              {props.repeatConstant.map((val, idx) => {
                return (
                  <Checkbox
                    key={`repeat-${idx}`}
                    value={val.value}
                    className={`${idx != props.repeatConstant.length - 1 ? ' mb-5' : ''}`}
                  >
        ...
        <AccordionCustome>
          <AccordionItemCustome title="Status Notifikasi">
            <Checkbox.Group
              className="mt-3 py-3 px-4"
              vertical
            ...
            >
              {OPTION_STATUS_NOTIFICATION.map((val, idx) => {
                return (
                  <Checkbox
                    key={idx}
                    value={val.value}
                    className={`${idx != OPTION_STATUS_NOTIFICATION.length - 1 ? ' mb-5' : ''}`}
                  >
                    <span className="text-center text-black text-sm font-normal font-goth leading-[21px]">
                      {val.label}
                    </span>
