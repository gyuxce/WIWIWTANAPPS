...
export const Tools = (props) => {
  ...
  const handleExport = async () => {
    setIsExporting(true);
    await exportData('xlsx', apiExport, 'notifikasi', props.localState.params);
  ...
  return (
    <>
        ...
        <div className="flex justify-end items-center space-x-[0.5rem]">
          ...
          {PageConfig.enableCreate && (
            <Button
              ...
              className="bg-white !px-3 !py-2 flex items-center !rounded"
            >
              <div className="mt-1 ml-1 !font-goth text-black !font-normal text-sm">Buat Notifikasi</div>
            </Button>
          )}
        </div>
      </div>
      ...
      <Filter
        ...
        repeatConstant={props.repeatConstant}
      />
    </>
  );
};
