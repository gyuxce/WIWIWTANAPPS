...
import { STATUS_PRA_CHARACTER } from 'components/ui/utils/constant';
...
export const Filter = (props) => {
  ...
  const submitFilter = async () => {
    ...
    if (startDate) {
      params.options.push(
        'filter,finished_at,greater_than_equal,' + dayjs(startDate).format('YYYY-MM-DD') + ' 00:00:00',
      );
    }
    if (endDate) {
      ...
      <div className="h-[85%] 2xl:h-[88%] overflow-auto">
        <AccordionCustome>
          <AccordionItemCustome title="Status Penilaian">
            ...
            >
              {STATUS_PRA_CHARACTER.map((val, idx) => {
                return (
                  <Checkbox
                    key={idx}
                    value={val.value}
                    className={`${idx != STATUS_PRA_CHARACTER.length - 1 ? ' mb-5' : ''}`}
                  >
