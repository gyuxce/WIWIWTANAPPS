...
export const Filter = (props) => {
  const [checkboxListSchedule, setCheckboxListSchedule] = useState([]);
  ...
  const submitFilter = async () => {
    ...
    if (startDate) {
      params.options.push(
        'filter,working_date,greater_than_equal,' + dayjs(startDate).format('YYYY-MM-DD') + ' 00:00:00',
      );
    }
    if (endDate) {
      params.options.push('filter,working_date,less_then_equal,' + dayjs(endDate).format('YYYY-MM-DD') + ' 23:59:59');
    }
    if (checkboxListSchedule.length > 0) {
      params.options.push('filter,is_scheduled,in,' + checkboxListSchedule.join('|'));
    }
    props.getData(params);
    props.setCountFilter(checkboxListSchedule.length + (startDate || endDate ? 1 : 0));
  ...
  return (
    ...
    >
      <div className="h-[85%] 2xl:h-[88%] overflow-auto">
        <AccordionCustome>
          <AccordionItemCustome title="Status">
            <Checkbox.Group
              className="mt-3 py-3 px-4"
              vertical
              value={checkboxListSchedule}
              onChange={setCheckboxListSchedule}
            >
              {props.schedule.map((val, idx) => {
                return (
                  <Checkbox
                    key={idx}
                    value={val.value}
                    className={`${idx != props.schedule.length - 1 ? ' mb-5' : ''}`}
                  >
