...
export const Tools = (props) => {
  ...
  const submitFilter = async () => {
    ...
    if (checkboxListStatus.length > 0) {
      params.options.push('filter,type,in,' + checkboxListStatus.join('|'));
    }
  ...
  const handleExport = async () => {
    setIsExporting(true);
    await exportData('xlsx', apiExport, 'Category Modul', props.localState.params);
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-[1rem]">
          ...
          {PageConfig.enableSearchTools && (
            <TableSearch
              size="md"
              placeholder="Cari kategori modul"
